...
\title{PBO Workshop} 
\subtitle{Creating Data Visualizations With D3} 
...
\begin{frame}
    ...
    \begin{block}{"If ProtoVis and jQuery Had a Child it Would be d3" -Mike Bostock}
\pause
        \begin{itemize}
...
\pause
    \item cmd-shift-i in Safari?
\pause
    \item Google "firebug shortcut keys" for a sense of what can be done with these tools
    \item All the tools are similar, but too different for me to fully detail for you though
    \end{itemize}
\end{frame}
...
\pause
        \item Scripts: allows for breakpoints just like a classic IDE
\pause
        \item Console: a REPL to test JavaScript code
\pause
        \end{itemize}
...
\begin{frame}
    ...
    \item Although elements can be selected individually we're normally using operators on the whole set
        ...
        \item Operators can both get or set:
            \begin{itemize}
            \item attribute values: .attr()
            \item \underline{inner} html content: .html()
            \item a raw property: .property() (Some HTML elements have special properties that are not addressable using standard attributes or styles)
            \item CSS style property: .style()
            \item add or remove CSS classes: .classed() 
            \end{itemize}
        \end{itemize}
    \end{itemize}
\end{frame}
...
\begin{frame}
    ...
    \item Elements can be accessed directly
        ...
        \end{itemize}
    \item Selection are an array of arrays of elements in order to preserve hierarchical structure of subselections
    \end{itemize}
\end{frame}
